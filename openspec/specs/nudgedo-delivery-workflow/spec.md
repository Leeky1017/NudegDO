# Spec: nudgedo-delivery-workflow

## Purpose

Define NudgeDO collaboration and delivery workflow as enforceable gates (Issue → PR → Checks → Auto-merge), so changes remain auditable, reproducible, and spec-first.

## Requirements

### Requirement: Every change is Issue-gated and PR-delivered

All changes MUST be tracked by a GitHub Issue `#N` and MUST be merged via a PR that follows NudgeDO delivery hard gates:

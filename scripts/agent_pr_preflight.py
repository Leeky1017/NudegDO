...
def _open_prs_by_issue_number(open_prs: list[PullRequestSummary]) -> dict[int, list[PullRequestSummary]]:
...
def _deps_from_execution_plan(markdown: str) -> dict[int, set[int]]:
    ...
    for raw_line in markdown.splitlines():
        ...
        if deps:
            deps_by_issue.setdefault(subject, set()).update(deps)

    return deps_by_issue


def _load_execution_plan(repo_root: Path, path: str) -> str | None:
...
def _dependencies_for_issue(repo_root: Path, execution_plan_path: str, issue_number: int) -> list[int]:
    markdown = _load_execution_plan(repo_root, execution_plan_path)
    if not markdown:
        return []
...
def _blocked_dependencies(
    ...
    deps = _dependencies_for_issue(repo_root, execution_plan, issue_number)

    _print_header("Roadmap Dependencies")
    if not deps:
        plan_path = (repo_root / execution_plan).resolve()
        if plan_path.exists():
            print("OK: no hard dependencies found in execution plan")
        else:
            print(f"SKIP: execution plan not found: {execution_plan}")
        return []
...
def _args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="NudgeDO agent PR preflight: deps + file overlap.")
    parser.add_argument(
        "--base-ref",
        default="origin/main",
    ...
    parser.add_argument(
        "--execution-plan",
        default="openspec/specs/nudgedo-roadmap/execution_plan.md",
